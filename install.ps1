...
Write-Host "* Importing image as WSL distribution *"
$DistName = Read-Host -Prompt 'Enter name of WSL dist'
.\steps\02_install.ps1 $DistName
Remove-Item rootfs.tar
Write-Host "* Installing additional tools in image *"
.\steps\03_install_additional_images.ps1 $DistName
Write-Host "* Creating a disk for designs (kept in case update) *"
.\steps\04_create_designs_disk.ps1 $DistName
